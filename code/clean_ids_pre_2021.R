...
clean_ids <- function(data_file = NULL, data_root = here::here('data', 'ids'),
                      maladie = 'measles', language = c('English', 'French'),
                      year = as.numeric(format(Sys.Date(), '%Y')), max_week = NULL,
                      add_history = TRUE, save = TRUE, 
                      output_root = here::here('out')) {

  maladies <- switch(language,
                     English = rio::import(here::here('data-raw', 'dictionary_maladie_en.rds')),
                     French = rio::import(here::here('data-raw', 'dictionary_maladie_fr.rds')))

  # ARGUMENT VALIDATION ----------------------------------------------------------------------------
  if (!(maladie %in% maladies) {
  ...
  df <- df |>
    janitor::clean_names() |>
    dplyr::mutate(
      maladie = recode(maladie), ## finish RECODE
      across.(contains(c('mois', 'ans')), as.numeric))
  ...
  #TODO: switch to epiclean for names
  #df <- df %>%
    #epiclean::clean_names()
